...
void Building::spawnPerson(Person newPerson){
    //need floor number and the direction they are going
    int personFloorNum = newPerson.getCurrentFloor();
    int request = newPerson.getTargetFloor() - newPerson.getCurrentFloor();

    floors[personFloorNum].addPerson(newPerson, request);
...
void Building::update(Move move){
    ...
    if (move.isValidMove(elevators)) {
        ...
        int peoplePick[MAX_PEOPLE_PER_FLOOR];
        if (move.isPickupMove()) {
            //remove people that are being picked up from the current floor
            move.copyListOfPeopleToPickup(peoplePick);
            floors[elevators[move.getElevatorId()].getCurrentFloor()]
                .removePeople(peoplePick, move.getNumPeopleToPickup());
        }

        if (!move.isPassMove()) {
            //keep the elevator in service if not a pass
            elevators[move.getElevatorId()].serviceRequest(target);
        }
...
int Building::tick(Move move){
    if (move.isValidMove(elevators)) {
        time++;
        update(move);
        //keep track of those exploded
        int exploded = 0;

        //tick time for each elevator
        for (int i = 0; i < NUM_ELEVATORS; i++) {
            elevators[i].tick(time);
        }

        //tick time for each floor
